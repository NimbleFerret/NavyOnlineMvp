export class CronosConstants {

    public static readonly AksContractAddress = '';
    public static readonly NvyContractAddress = '';

    public static readonly CaptainContractAddress = '0x2f79860E2a2829AF3C135880da1e8fC3fD9AE398'.toLowerCase();
    public static readonly ShipContractAddress = '0x2f79860E2a2829AF3C135880da1e8fC3fD9AE399'.toLowerCase();
    public static readonly IslandContractAddress = '0x2f79860E2a2829AF3C135880da1e8fC3fD9AE310'.toLowerCase();

    public static readonly ShipTemplateContractAddress = '';

    public static readonly CaptainCollectionContractAddress = '0x16223935C5b2Ae50785604dDee8E350334805f05'.toLowerCase();
    public static readonly ShipCollectionContractAddress = '';
    public static readonly IslandCollectionContractAddress = '';

    public static readonly CaptainMarketplaceContractAddress = '0x59911FF9eFee39d96EbDeee940F28e26DccD8530'.toLowerCase();
    public static readonly ShipMarketplaceContractAddress = '';
    public static readonly IslandMarketplaceContractAddress = '';

}