...
export class NftShipGenerator {
    ...
    async generateFounderShip(index: number, maxIndex: number, shipStats: PlayerShipEntity) {
        ...
        const anchorRnd = RandomService.GetRandomIntInRange(1, 100);
        const oneWindowRnd = RandomService.GetRandomIntInRange(1, 100);
        const twoWindowRnd = RandomService.GetRandomIntInRange(1, 100);
