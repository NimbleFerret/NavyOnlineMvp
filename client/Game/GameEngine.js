...
var engine_GameEngine = $hx_exports["engine"]["GameEngine"] = function(engineMode) {
	...
	engine_BaseEngine.call(this,engineMode,engine_entity_GameEntityType.Ship,new engine_entity_manager_ShipManager());
};
engine_GameEngine.__name__ = true;
engine_GameEngine.main = function() {
};
engine_GameEngine.__super__ = engine_BaseEngine;
engine_GameEngine.prototype = $extend(engine_BaseEngine.prototype,{
	engineLoopUpdate: function(dt) {
		...
		while(!_g_lastStep.done) {
			...
			if(ship.isAlive) {
				...
				if(engineShipEntity.role == engine_entity_Role.Bot && engineShipEntity.allowShoot) {
					...
					this.shipShootBySide(engine_entity_Side.Right,engineShipEntity.id);
				}
				var jsIterator = this.mainEntityManager.entities.values();
				...
				if(shell.ownerId != ship.id) {
					if(shell.getGameRect().intersectsWithRect(ship.getGameRect()) && ship.isAlive) {
						...
						if(this.shipHitByShellCallback != null) {
							this.shipHitByShellCallback({ ship : engineShipEntity, damage : engineShellEntity.damage});
						}
		...
		while(_g < _g1) {
			var i = _g++;
			var shell = js_Boot.__cast(this.shellManager.getEntityById(shellsToDelete[i]) , engine_entity_EngineShellEntity);
			if(shell != null) {
				if(this.deleteShellCallback != null) {
					this.deleteShellCallback(shell);
				}
				this.shellManager.remove(shell.id);
			}
		}
		var _g = 0;
		...
		while(_g < _g1) {
			var i = _g++;
			var ship = js_Boot.__cast(this.mainEntityManager.getEntityById(shipsToDelete[i]) , engine_entity_EngineShipEntity);
			if(ship != null) {
				this.removeMainEntity(ship.id);
			}
		}
	}
	...
	,createEntity: function(serverShipRef,free,role,x,y,size,windows,cannons,cannonsRange,cannonsDamage,armor,hull,maxSpeed,acc,accDelay,turnDelay,fireDelay,id,ownerId) {
		var entity = new engine_entity_EngineShipEntity(serverShipRef,free,role,x,y,size,windows,cannons,cannonsRange,cannonsDamage,armor,hull,maxSpeed,acc,accDelay,turnDelay,fireDelay,id,ownerId);
		this.createMainEntity(entity,true);
		return entity;
	}
	,entityMoveUp: function(id) {
		var ship = js_Boot.__cast(this.mainEntityManager.getEntityById(id) , engine_entity_EngineShipEntity);
		if(ship != null) {
	...
	,shipShootBySide: function(side,shipId,serverSide,shellRnd) {
		...
		var ship = js_Boot.__cast(this.mainEntityManager.getEntityById(shipId) , engine_entity_EngineShipEntity);
		if(ship != null && ship.tryShoot(side)) {
			...
			var shell1 = this.addShell(side,0,pos1.x,pos1.y,ship.cannonsDamage,ship.cannonsRange,shipSideRadRotation,ship.id,shellRnd != null && shellRnd[0] != null ? shellRnd[0] : null);
			shell1.serverSide = serverSide;
			var shells = [shell1];
			if(ship.shipGuns == engine_entity_ShipGuns.TWO) {
				var pos2 = ship.getCanonOffsetBySideAndIndex(side,1);
				var shell2 = this.addShell(side,1,pos2.x,pos2.y,ship.cannonsDamage,ship.cannonsRange,shipSideRadRotation,ship.id,shellRnd != null && shellRnd[1] != null ? shellRnd[1] : null);
				shell2.serverSide = serverSide;
				shells.push(shell2);
			}
			if(ship.shipGuns == engine_entity_ShipGuns.THREE) {
				var pos2 = ship.getCanonOffsetBySideAndIndex(side,1);
				var pos3 = ship.getCanonOffsetBySideAndIndex(side,2);
				var shell2 = this.addShell(side,1,pos2.x,pos2.y,ship.cannonsDamage,ship.cannonsRange,shipSideRadRotation,ship.id,shellRnd != null && shellRnd[1] != null ? shellRnd[1] : null);
				var shell3 = this.addShell(side,2,pos3.x,pos3.y,ship.cannonsDamage,ship.cannonsRange,shipSideRadRotation,ship.id,shellRnd != null && shellRnd[2] != null ? shellRnd[2] : null);
				shell2.serverSide = serverSide;
				shell3.serverSide = serverSide;
				shells.push(shell2);
				shells.push(shell3);
			}
			if(ship.shipGuns == engine_entity_ShipGuns.FOUR) {
				var pos2 = ship.getCanonOffsetBySideAndIndex(side,1);
				var pos3 = ship.getCanonOffsetBySideAndIndex(side,2);
				var pos4 = ship.getCanonOffsetBySideAndIndex(side,3);
				var shell2 = this.addShell(side,1,pos2.x,pos2.y,ship.cannonsDamage,ship.cannonsRange,shipSideRadRotation,ship.id,shellRnd != null && shellRnd[1] != null ? shellRnd[1] : null);
				var shell3 = this.addShell(side,2,pos3.x,pos3.y,ship.cannonsDamage,ship.cannonsRange,shipSideRadRotation,ship.id,shellRnd != null && shellRnd[2] != null ? shellRnd[2] : null);
				var shell4 = this.addShell(side,3,pos4.x,pos4.y,ship.cannonsDamage,ship.cannonsRange,shipSideRadRotation,ship.id,shellRnd != null && shellRnd[3] != null ? shellRnd[3] : null);
				shell2.serverSide = serverSide;
				shell3.serverSide = serverSide;
	...
	,addShell: function(side,pos,x,y,damage,range,rotation,ownerId,shellRnd) {
		var newShell = new engine_entity_EngineShellEntity(side,pos,x,y,rotation,damage,range,ownerId,shellRnd);
...
var engine_entity_DieEffect = $hxEnums["engine.entity.DieEffect"] = { __ename__:true,__constructs__:null
...
var engine_entity_EngineShellEntity = function(side,pos,x,y,rotation,damage,range,ownerId,shellRnd) {
...
engine_entity_EngineShellEntity.__interfaces__ = [engine_entity_GameEntityCustomCollide,engine_entity_GameEntityCustomUpdate];
...
var engine_entity_Role = $hxEnums["engine.entity.Role"] = { __ename__:true,__constructs__:null
	,Bot: {_hx_name:"Bot",_hx_index:0,__enum__:"engine.entity.Role",toString:$estr}
	,Boss: {_hx_name:"Boss",_hx_index:1,__enum__:"engine.entity.Role",toString:$estr}
	,Player: {_hx_name:"Player",_hx_index:2,__enum__:"engine.entity.Role",toString:$estr}
};
engine_entity_Role.__constructs__ = [engine_entity_Role.Bot,engine_entity_Role.Boss,engine_entity_Role.Player];
var engine_entity_ShipHullSize = $hxEnums["engine.entity.ShipHullSize"] = { __ename__:true,__constructs__:null
	,SMALL: {_hx_name:"SMALL",_hx_index:0,__enum__:"engine.entity.ShipHullSize",toString:$estr}
	,MEDIUM: {_hx_name:"MEDIUM",_hx_index:1,__enum__:"engine.entity.ShipHullSize",toString:$estr}
};
engine_entity_ShipHullSize.__constructs__ = [engine_entity_ShipHullSize.SMALL,engine_entity_ShipHullSize.MEDIUM];
var engine_entity_ShipWindows = $hxEnums["engine.entity.ShipWindows"] = { __ename__:true,__constructs__:null
	,ONE: {_hx_name:"ONE",_hx_index:0,__enum__:"engine.entity.ShipWindows",toString:$estr}
	,TWO: {_hx_name:"TWO",_hx_index:1,__enum__:"engine.entity.ShipWindows",toString:$estr}
	,NONE: {_hx_name:"NONE",_hx_index:2,__enum__:"engine.entity.ShipWindows",toString:$estr}
};
engine_entity_ShipWindows.__constructs__ = [engine_entity_ShipWindows.ONE,engine_entity_ShipWindows.TWO,engine_entity_ShipWindows.NONE];
var engine_entity_ShipGuns = $hxEnums["engine.entity.ShipGuns"] = { __ename__:true,__constructs__:null
	,ONE: {_hx_name:"ONE",_hx_index:0,__enum__:"engine.entity.ShipGuns",toString:$estr}
	,TWO: {_hx_name:"TWO",_hx_index:1,__enum__:"engine.entity.ShipGuns",toString:$estr}
	,THREE: {_hx_name:"THREE",_hx_index:2,__enum__:"engine.entity.ShipGuns",toString:$estr}
	,FOUR: {_hx_name:"FOUR",_hx_index:3,__enum__:"engine.entity.ShipGuns",toString:$estr}
};
engine_entity_ShipGuns.__constructs__ = [engine_entity_ShipGuns.ONE,engine_entity_ShipGuns.TWO,engine_entity_ShipGuns.THREE,engine_entity_ShipGuns.FOUR];
var engine_entity_EngineShipEntity = function(serverShipRef,free,role,x,y,shipHullSize,shipWindows,shipGuns,cannonsRange,cannonsDamage,armor,hull,maxSpeed,acc,accDelay,turnDelay,fireDelay,id,ownerId) {
	this.allowShoot = false;
	...
	this.timeSinceLastShipsPosUpdate = 0.0;
	this.fireDelay = 0.200;
	this.turnDelay = 1.0;
	this.accDelay = 1.0;
	this.cannonsDamage = 50;
	this.cannonsRange = 600;
	...
	if(shipHullSize == engine_entity_ShipHullSize.MEDIUM) {
		this.shapeWidth = 300;
...
engine_entity_EngineShipEntity.prototype = $extend(engine_entity_EngineBaseGameEntity.prototype,{
	checkMovementInput: function() {
		...
		if(this.lastMovementInputCheck == 0 || this.lastMovementInputCheck + this.accDelay < now) {
	...
	,checkRotationInput: function() {
		var now = HxOverrides.now() / 1000;
		if(this.lastRotationInputCheck == 0 || this.lastRotationInputCheck + this.turnDelay < now) {
	...
	,rotateLeft: function() {
		if(this.checkRotationInput()) {
			...
			if(this.directionChangeCallbackLeft != null) {
				this.directionChangeCallbackLeft(this.direction);
			}
			return true;
		} else {
			return false;
	...
	,rotateRight: function() {
		if(this.checkRotationInput()) {
			...
			if(this.directionChangeCallbackRight != null) {
				this.directionChangeCallbackRight(this.direction);
			}
			return true;
		} else {
			return false;
	...
	,shootAllowanceBySide: function(side) {
		var now = HxOverrides.now() / 1000;
		if(side == engine_entity_Side.Right) {
			if(this.lastRightShootInputCheck != 0) {
				return this.lastRightShootInputCheck + this.fireDelay < now;
			} else {
				return true;
			}
		} else if(this.lastLeftShootInputCheck != 0) {
			return this.lastLeftShootInputCheck + this.fireDelay < now;
		} else {
			return true;
		}
	}
	,tryShoot: function(side) {
		var now = HxOverrides.now() / 1000;
		if(side == engine_entity_Side.Right) {
			if(this.lastRightShootInputCheck == 0 || this.lastRightShootInputCheck + this.fireDelay < now) {
		...
		} else if(this.lastLeftShootInputCheck == 0 || this.lastLeftShootInputCheck + this.fireDelay < now) {
	...
	,getCanonOffsetBySideAndIndex: function(side,index) {
		...
		if(side == engine_entity_Side.Left) {
			if(this.direction == engine_entity_GameEntityDirection.East) {
				additionalOffsetX = 0;
				additionalOffsetY = 20;
			}
			if(this.direction == engine_entity_GameEntityDirection.NorthEast) {
				additionalOffsetX = 14;
				additionalOffsetY = 8;
			}
			if(this.direction == engine_entity_GameEntityDirection.North) {
				additionalOffsetX = 20;
				additionalOffsetY = 0;
			}
			if(this.direction == engine_entity_GameEntityDirection.NorthWest) {
				additionalOffsetX = 14;
				additionalOffsetY = -8;
			}
			if(this.direction == engine_entity_GameEntityDirection.West) {
				additionalOffsetX = 0;
				additionalOffsetY = -20;
			}
			if(this.direction == engine_entity_GameEntityDirection.SouthWest) {
				additionalOffsetX = -14;
				additionalOffsetY = -8;
			}
			if(this.direction == engine_entity_GameEntityDirection.SouthEast) {
				additionalOffsetX = -14;
				additionalOffsetY = 8;
			}
			if(this.direction == engine_entity_GameEntityDirection.South) {
				additionalOffsetX = -20;
				additionalOffsetY = 0;
			}
		} else {
		...
		if(this.shipHullSize == engine_entity_ShipHullSize.MEDIUM) {
			offset = side == engine_entity_Side.Left ? engine_entity_EngineShipEntity.LeftCanonsOffsetByDirMid.get(this.direction) : engine_entity_EngineShipEntity.RightCanonsOffsetByDirMid.get(this.direction);
		} else {
			offset = side == engine_entity_Side.Left ? engine_entity_EngineShipEntity.LeftCanonsOffsetByDirSm.get(this.direction) : engine_entity_EngineShipEntity.RightCanonsOffsetByDirSm.get(this.direction);
		}
...
engine_entity_EngineShipEntity.RightCanonsOffsetByDirSm = (function($this) {
	var $r;
	var _g = new haxe_ds_EnumValueMap();
	_g.set(engine_entity_GameEntityDirection.East,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-28,26),new engine_entity_PosOffset(0,26),new engine_entity_PosOffset(28,26)));
	_g.set(engine_entity_GameEntityDirection.NorthEast,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(17,31),new engine_entity_PosOffset(36,21),new engine_entity_PosOffset(55,13)));
	_g.set(engine_entity_GameEntityDirection.North,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(50,10),new engine_entity_PosOffset(50,-13),new engine_entity_PosOffset(50,33)));
	_g.set(engine_entity_GameEntityDirection.NorthWest,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(50,-16),new engine_entity_PosOffset(28,-26),new engine_entity_PosOffset(7,-33)));
	_g.set(engine_entity_GameEntityDirection.West,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(50,-50),new engine_entity_PosOffset(21,-50),new engine_entity_PosOffset(-8,-50)));
	_g.set(engine_entity_GameEntityDirection.SouthWest,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-10,-41),new engine_entity_PosOffset(-27,-34),new engine_entity_PosOffset(-41,-27)));
	_g.set(engine_entity_GameEntityDirection.South,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-50,-31),new engine_entity_PosOffset(-50,-11),new engine_entity_PosOffset(-50,10)));
	_g.set(engine_entity_GameEntityDirection.SouthEast,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-70,10),new engine_entity_PosOffset(-50,19),new engine_entity_PosOffset(-28,29)));
	$r = _g;
	return $r;
}(this));
engine_entity_EngineShipEntity.LeftCanonsOffsetByDirSm = (function($this) {
	var $r;
	var _g = new haxe_ds_EnumValueMap();
	_g.set(engine_entity_GameEntityDirection.East,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-28,-52),new engine_entity_PosOffset(0,-52),new engine_entity_PosOffset(28,-52)));
	_g.set(engine_entity_GameEntityDirection.NorthEast,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-55,-16),new engine_entity_PosOffset(-35,-24),new engine_entity_PosOffset(-16,-32)));
	_g.set(engine_entity_GameEntityDirection.North,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-50,10),new engine_entity_PosOffset(-50,-13),new engine_entity_PosOffset(-50,33)));
	_g.set(engine_entity_GameEntityDirection.NorthWest,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-13,32),new engine_entity_PosOffset(-32,22),new engine_entity_PosOffset(-52,11)));
	_g.set(engine_entity_GameEntityDirection.West,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(50,27),new engine_entity_PosOffset(21,27),new engine_entity_PosOffset(-8,27)));
	_g.set(engine_entity_GameEntityDirection.SouthWest,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(70,5),new engine_entity_PosOffset(50,15),new engine_entity_PosOffset(31,25)));
	_g.set(engine_entity_GameEntityDirection.South,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(50,-31),new engine_entity_PosOffset(50,-11),new engine_entity_PosOffset(50,10)));
	_g.set(engine_entity_GameEntityDirection.SouthEast,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(11,-32),new engine_entity_PosOffset(27,-25),new engine_entity_PosOffset(46,-15)));
	$r = _g;
	return $r;
}(this));
engine_entity_EngineShipEntity.RightCanonsOffsetByDirMid = (function($this) {
	var $r;
	var _g = new haxe_ds_EnumValueMap();
	_g.set(engine_entity_GameEntityDirection.East,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-49,54),new engine_entity_PosOffset(-19,54),new engine_entity_PosOffset(11,54),new engine_entity_PosOffset(41,54)));
	_g.set(engine_entity_GameEntityDirection.NorthEast,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(21,51),new engine_entity_PosOffset(47,38),new engine_entity_PosOffset(72,25),new engine_entity_PosOffset(96,12)));
	_g.set(engine_entity_GameEntityDirection.North,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(47,8),new engine_entity_PosOffset(47,-13),new engine_entity_PosOffset(47,-34),new engine_entity_PosOffset(47,-55)));
	_g.set(engine_entity_GameEntityDirection.NorthWest,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(27,-17),new engine_entity_PosOffset(5,-28),new engine_entity_PosOffset(-19,-40),new engine_entity_PosOffset(-42,-51)));
	_g.set(engine_entity_GameEntityDirection.West,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(0,-59),new engine_entity_PosOffset(-24,-59),new engine_entity_PosOffset(-51,-59),new engine_entity_PosOffset(-78,-59)));
	_g.set(engine_entity_GameEntityDirection.SouthWest,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-61,-28),new engine_entity_PosOffset(-82,-17),new engine_entity_PosOffset(-103,-6),new engine_entity_PosOffset(-125,5)));
	_g.set(engine_entity_GameEntityDirection.South,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-87,-10),new engine_entity_PosOffset(-87,10),new engine_entity_PosOffset(-87,30),new engine_entity_PosOffset(-87,50)));
	_g.set(engine_entity_GameEntityDirection.SouthEast,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-82,33),new engine_entity_PosOffset(-60,44),new engine_entity_PosOffset(-39,54),new engine_entity_PosOffset(-18,64)));
	$r = _g;
	return $r;
}(this));
engine_entity_EngineShipEntity.LeftCanonsOffsetByDirMid = (function($this) {
	var $r;
	var _g = new haxe_ds_EnumValueMap();
	_g.set(engine_entity_GameEntityDirection.East,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-43,-59),new engine_entity_PosOffset(-15,-59),new engine_entity_PosOffset(13,-59),new engine_entity_PosOffset(41,-59)));
	_g.set(engine_entity_GameEntityDirection.NorthEast,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-63,-20),new engine_entity_PosOffset(-43,-30),new engine_entity_PosOffset(-23,-40),new engine_entity_PosOffset(-3,-50)));
	_g.set(engine_entity_GameEntityDirection.North,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-87,8),new engine_entity_PosOffset(-87,-13),new engine_entity_PosOffset(-87,-34),new engine_entity_PosOffset(-87,-55)));
	_g.set(engine_entity_GameEntityDirection.NorthWest,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(-60,48),new engine_entity_PosOffset(-81,38),new engine_entity_PosOffset(-102,28),new engine_entity_PosOffset(-123,18)));
	_g.set(engine_entity_GameEntityDirection.West,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(0,54),new engine_entity_PosOffset(-24,54),new engine_entity_PosOffset(-51,54),new engine_entity_PosOffset(-78,54)));
	_g.set(engine_entity_GameEntityDirection.SouthWest,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(42,36),new engine_entity_PosOffset(24,44),new engine_entity_PosOffset(-3,59),new engine_entity_PosOffset(-26,70)));
	_g.set(engine_entity_GameEntityDirection.South,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(48,-10),new engine_entity_PosOffset(48,10),new engine_entity_PosOffset(48,30),new engine_entity_PosOffset(48,50)));
	_g.set(engine_entity_GameEntityDirection.SouthEast,new engine_entity_PosOffsetArray(new engine_entity_PosOffset(25,-26),new engine_entity_PosOffset(47,-16),new engine_entity_PosOffset(70,-4),new engine_entity_PosOffset(93,6)));
	$r = _g;
	return $r;
}(this));
